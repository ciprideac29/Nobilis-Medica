...
const googleAdsConversionTarget = "AW-18229543551/RsNVCLWm37wcEP-Ew_RD";
const googleFormEntries = {
  "Nume și prenume": "entry.2136236739",
  Telefon: "entry.1861786703",
  Email: "entry.279673329",
  "Cum te putem ajuta?": "entry.1355932853",
  Mesaj: "entry.236382902",
  "Acord prelucrare date": "entry.890030238",
};
...
function trackGoogleAdsConversion(callback) {
  ...
  if (!googleAdsConversionTarget || typeof window.gtag !== "function") {
    finish();
    return;
  }
...
document.querySelectorAll("[data-contact-form]").forEach((form) => {
  form.addEventListener("submit", (event) => {
    ...
    const googleFormAction = form.dataset.googleFormAction;
    if (googleFormAction) {
      ...
      Object.entries(googleFormEntries).forEach(([fieldName, entryName]) => {
        googleFormData.append(entryName, data.get(fieldName) || "");
      });
      ...
      fetch(googleFormAction, {
        ...
        .then(() => {
          ...
          if (status) {
            status.textContent =
              "Solicitarea a fost trimisa. Va vom contacta in cel mai scurt timp.";
        ...
        .catch(() => {
          if (status) {
            status.textContent =
              "Solicitarea nu a putut fi trimisa. Te rugam sa ne contactezi telefonic sau pe email.";
    ...
    const lines = [
      ...
      `Acord prelucrare date: ${data.get("Acord prelucrare date") || "Nu"}`,
      "",
      "Mesaj:",
      `${data.get("Mesaj") || ""}`,
    ];
